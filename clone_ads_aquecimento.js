var DATA = {
  "name": "aquecimento INSTA - Clone " + new Date().toISOString().slice(0, 10),
  "budget": 0.50,
  ...
  "adGroups": [
    {
      ...
      "ads": [
        {
          "finalUrl": "http://instagram.com/kimmorc",
          "headlines": [
            "Instagram",
...
function main() {
  Logger.log("=== INICIANDO CRIAÇÃO VIA BULK UPLOAD (CORRIGIDO) ===");

  // CORREÇÃO: Adicionadas colunas obrigatórias "Campaign type", "Campaign status" e "EU political ads"
  var columns = [
    "Action", 
    "Campaign", 
    "Budget", 
    ...
    "Campaign type", 
    "Campaign status",
    "EU political ads", // <--- COLUNA NOVA ADICIONADA
    "Ad Group", 
    "Keyword", 
    "Match Type", 
    "Ad type", 
  ...
  upload.append({
    ...
    "Campaign type": DATA.channelType, // "Search"
    "Campaign status": "Enabled",
    "EU political ads": "No" // <--- CORREÇÃO: "No" equivale a DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING
  });
  ...
  DATA.adGroups.forEach(function(group) {
    ...
    upload.append({
      ...
      "Campaign": DATA.name,
      "Ad Group": group.name,
      "Campaign status": "Enabled" // Reforço de status
    });
  ...
  Logger.log("Dados corrigidos preparados. Enviando...");
  
  upload.apply();

  Logger.log("=== PROCESSO CONCLUÍDO ===");
  Logger.log("Verifique novamente a aba 'Uploads em massa'.");
}
